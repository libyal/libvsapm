...
/* The pyvsapm module methods
 */
PyMethodDef pyvsapm_module_methods[] = {
	{ "get_version",
	  (PyCFunction) pyvsapm_get_version,
	  METH_NOARGS,
	  "get_version() -> String\n"
	  "\n"
	...
	{ "check_volume_signature",
	  (PyCFunction) pyvsapm_check_volume_signature,
	  METH_VARARGS | METH_KEYWORDS,
	  "check_volume_signature(filename) -> Boolean\n"
	  "\n"
	  "Checks if a volume has a Apple Partition Map (APM) volume system signature." },

	{ "check_volume_signature_file_object",
	  (PyCFunction) pyvsapm_check_volume_signature_file_object,
	  METH_VARARGS | METH_KEYWORDS,
	  "check_volume_signature_file_object(file_object) -> Boolean\n"
	  "\n"
	  "Checks if a volume has a Apple Partition Map (APM) volume system signature using a file-like object." },

	{ "open",
	  (PyCFunction) pyvsapm_open_new_volume,
	  METH_VARARGS | METH_KEYWORDS,
	...
	{ "open_file_object",
	  (PyCFunction) pyvsapm_open_new_volume_with_file_object,
	  METH_VARARGS | METH_KEYWORDS,
...
/* Retrieves the pyvsapm/libvsapm version
 * Returns a Python object if successful or NULL on error
 */
PyObject *pyvsapm_get_version(
           PyObject *self PYVSAPM_ATTRIBUTE_UNUSED,
           PyObject *arguments PYVSAPM_ATTRIBUTE_UNUSED )
{
	const char *version_string   = NULL;
	size_t version_string_length = 0;

	PYVSAPM_UNREFERENCED_PARAMETER( self )
           ...
           PyObject *keywords )
{
	...
	static char *function       = "pyvsapm_check_volume_signature";
           ...
           PyObject *keywords )
{
	...
	libcerror_error_t *error         = NULL;
	static char *function            = "pyvsapm_check_volume_signature_file_object";
...
PyObject *pyvsapm_open_new_volume(
           ...
           PyObject *keywords )
{
	pyvsapm_volume_t *pyvsapm_volume = NULL;
	static char *function            = "pyvsapm_open_new_volume";

	PYVSAPM_UNREFERENCED_PARAMETER( self )

	/* PyObject_New does not invoke tp_init
	 */
	pyvsapm_volume = PyObject_New(
	                  struct pyvsapm_volume,
	                  &pyvsapm_volume_type_object );
	...
	if( pyvsapm_volume_init(
	     pyvsapm_volume ) != 0 )
	{
		goto on_error;
	}
	if( pyvsapm_volume_open(
	     pyvsapm_volume,
	     arguments,
	     keywords ) == NULL )
	{
	...
	return( (PyObject *) pyvsapm_volume );
...
{
	pyvsapm_volume_t *pyvsapm_volume = NULL;
	static char *function            = "pyvsapm_open_new_volume_with_file_object";
	...
	if( pyvsapm_volume_open_file_object(
	     pyvsapm_volume,
	     arguments,
	     keywords ) == NULL )
	{
...
/* The pyvsapm module definition
 */
PyModuleDef pyvsapm_module_definition = {
	PyModuleDef_HEAD_INIT,

	/* m_name */
	"pyvsapm",
	/* m_doc */
	"Python libvsapm module (pyvsapm).",
	/* m_size */
	-1,
	/* m_methods */
	pyvsapm_module_methods,
	/* m_reload */
	NULL,
	/* m_traverse */
	NULL,
...
/* Initializes the pyvsapm module
 */
#if PY_MAJOR_VERSION >= 3
...
#endif
{
	 ...
	 */
	pyvsapm_partition_type_object.tp_new = PyType_GenericNew;

	if( PyType_Ready(
	     &pyvsapm_partition_type_object ) < 0 )
	{
		goto on_error;
	}
	Py_IncRef(
	 (PyObject *) &pyvsapm_partition_type_object );
	 ...
	 */
	pyvsapm_partitions_type_object.tp_new = PyType_GenericNew;

	if( PyType_Ready(
	     &pyvsapm_partitions_type_object ) < 0 )
	{
		goto on_error;
	}
	Py_IncRef(
	 (PyObject *) &pyvsapm_partitions_type_object );
	 ...
	 */
	pyvsapm_volume_type_object.tp_new = PyType_GenericNew;

	if( PyType_Ready(
	     &pyvsapm_volume_type_object ) < 0 )
	{
		goto on_error;
	}
	Py_IncRef(
	 (PyObject *) &pyvsapm_volume_type_object );

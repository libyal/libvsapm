...
#include "../libvsapm/libvsapm_section_values.h"

#if defined( __GNUC__ ) && !defined( LIBVSAPM_DLL_IMPORT )

/* Tests the libvsapm_section_values_initialize function
 * Returns 1 if successful or 0 if not
 */
int vsapm_test_section_values_initialize(
     void )
{
	libcerror_error_t *error                  = NULL;
	libvsapm_section_values_t *section_values = NULL;
	int result                                = 0;
	...
	/* Test regular cases
	 */
	result = libvsapm_section_values_initialize(
	          &section_values,
	          &error );
	...
	result = libvsapm_section_values_free(
	          &section_values,
	          &error );
	...
	section_values = (libvsapm_section_values_t *) 0x12345678UL;
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libvsapm_section_values_initialize with malloc failing
		 */
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libvsapm_section_values_initialize with memset failing
		 */
...
/* Tests the libvsapm_section_values_free function
 * Returns 1 if successful or 0 if not
 */
int vsapm_test_section_values_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
#endif
{
	...
	VSAPM_TEST_RUN(
	 "libvsapm_section_values_initialize",
	 vsapm_test_section_values_initialize );

	VSAPM_TEST_RUN(
	 "libvsapm_section_values_free",
	 vsapm_test_section_values_free );

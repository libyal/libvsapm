...
#include "../vsapmtools/info_handle.h"

/* Tests the info_handle_initialize function
 * Returns 1 if successful or 0 if not
 */
int vsapm_test_tools_info_handle_initialize(
     void )
{
 ...
 * Returns 1 if successful or 0 if not
 */
int vsapm_test_tools_info_handle_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
#endif
{
	...
	VSAPM_TEST_RUN(
	 ...
	 vsapm_test_tools_info_handle_initialize );

	VSAPM_TEST_RUN(
	 "info_handle_free",
	 vsapm_test_tools_info_handle_free );
